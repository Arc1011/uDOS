...
void initFS() {
  ...
  char devPath[PATH_LEN] = "/";
  const char* pins[] = {"COM2", "COM3", "COM4"};
  for (d = 0; d < 3; d++) {
    for (i = 0; i < MAX_FILES; i++) {
      if (!fs[i].active) {
...
void executeCommand(char* line) {
  ...
  if (strcmp_P(cmd, PSTR("pinmode")) == 0) {
  ...
  else if (strcmp_P(cmd, PSTR("dir")) == 0) {
    int empty = 1, j;
  ...
  else if (strcmp_P(cmd, PSTR("md")) == 0 || strcmp_P(cmd, PSTR("mkf")) == 0) {
    int foundSlot = -1, j;
    for (j = 0; j < MAX_FILES; j++) {
    ...
    fs[foundSlot].isDirectory = (strcmp_P(cmd, PSTR("md")) == 0);
  ...
  else if (strcmp_P(cmd, PSTR("path")) == 0) {
    ...
    if (arrow != -1) {
      ...
      for (j = 0; j < MAX_FILES; j++) {
        if (fs[j].active && !fs[j].isDirectory &&
          ...
          if (strcmp_P(fs[j].parentDir, PSTR("/")) == 0 && strncmp_P(fs[j].name, PSTR("COM"), 3) == 0) {
  ...
  else if (strcmp_P(cmd, PSTR("type")) == 0) {
    int j, found = 0;
    for (j = 0; j < MAX_FILES; j++) {
      if (fs[j].active && !fs[j].isDirectory &&
          strcmp(args, fs[j].name) == 0 &&
  ...
  else if (strcmp_P(cmd, PSTR("attrib")) == 0) {
    int j, found = 0;
    for (j = 0; j < MAX_FILES; j++) {
  ...
  else if (strcmp_P(cmd, PSTR("del")) == 0) {
    int j, found = 0;
    for (j = 0; j < MAX_FILES; j++) {
      if (fs[j].active && strcmp(args, fs[j].name) == 0 && strcmp(fs[j].parentDir, currentPath) == 0) {
        if (fs[j].isDirectory) {
  ...
  else if (strcmp_P(cmd, PSTR("syslog")) == 0) {
    Serial.println(F("=== SYSTEM LOG ==="));
    int j;
  ...
  else if (strcmp_P(cmd, PSTR("cls")) == 0) {
    int j;
    for (j = 0; j < 30; j++) Serial.println();
  }
  else if (strcmp_P(cmd, PSTR("bat")) == 0) {
    if (args[0] == '\0') {
      Serial.println(F("Usage: bat [script]")); return;
    }
    int j, found = 0;
    for (j = 0; j < MAX_FILES; j++) {
      if (fs[j].active && !fs[j].isDirectory &&
  ...
  else if (strcmp_P(cmd, PSTR("doskey")) == 0) {
    if (args[0] == '\0') {
      int j, any = 0;
  ...
  else if (strcmp_P(cmd, PSTR("search")) == 0) {
    if (args[0] == '\0') { Serial.println(F("Usage: search [name]")); return; }
    int j, found = 0;
    for (j = 0; j < MAX_FILES; j++) {
    ...
    Serial.println(F("Commands: dir, cd, path, md, mkf, type, echo, del, attrib"));
    Serial.println(F("          pinmode, write, read, gpio, pwm, bat"));
    Serial.println(F("          uptime, ver, syslog, mem, whoami, cls, reboot"));
    Serial.println(F("          doskey, slots, search"));
    Serial.println(F("GPIO: gpio [pin] on/off/toggle  |  gpio vixa [count]"));
    Serial.println(F("BAT:  bat [file]  -- run script (use ; as line separator)"));
  }
  else {
    // check alias
    int j, resolved = 0;
...
// Interpreter bat (sh from KernelUNO)
void runScript(const char* content) {
  char line[32];
  ...
  while (ci <= len) {
    ...
    if (c == ';' || c == '\n' || c == '\r') {
      if (li > 0) {
        line[li] = '\0';
        lineNum++;
        Serial.print(F("[bat:")); Serial.print(lineNum); Serial.print(F("] "));
        Serial.println(line);
        executeCommand(line);
        li = 0;
      }
  ...
  addDmesg(F("bat: script done"));
  Serial.println(F("[bat] done."));
}
